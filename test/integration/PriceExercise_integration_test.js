...
skip.if(developmentChains.includes(network.name)).
  describe('PriceExercise Integration Tests', async function () {
     //Rename the APIConsumer variable declared under the ‘describe’ line to ‘priceExercise’
      let priceExercise;

      beforeEach(async () => {
          const PriceExercise = await deployments.get('PriceExercise');
          apiConsumer = await ethers.getContractAt('PriceExercise', PriceExercise.address)
      });
    ...
    it('Should successfully make an external API request and get a result', async () => {
      ...
      //Now check the result
        const result = await apriceExercise.storedPrice();
